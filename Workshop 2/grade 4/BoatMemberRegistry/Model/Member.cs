    ...
    public class Member : BoatObserver
    {
        ...
        public string Name
        {
            ...
            set
            {
                ...
                if (trimmed.Length > 2)
                {
                    if (Regex.IsMatch(value, @"^[âãäåæçèéêëìíîïðñòóôõøùúûüýþÿa-zA-Z -]+$"))
                    {
                        if (m_name != trimmed)
                        {
                            m_name = trimmed;
                    ...
                    {
                        throw new Exception("Member name can only contain letters (and space)");
                    }
                }
                else
                {
        ...
        public void MemberChangeMade(Model.Boat a_boat)
        {
